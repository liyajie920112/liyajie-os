...
const modules: Record<string, { default: unknown }> = import.meta.glob('./modules/**/*.ts', { eager: true });
...
export const getActive = (route: RouteLocationNormalized, maxLevel = 3): string => {
  if (!route.path) {
    return '';
  }
  return route.path
